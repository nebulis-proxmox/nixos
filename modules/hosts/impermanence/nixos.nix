...
}:
{
  ...
  config = {
    nebulis.impermanence.backup = lib.mkIf config.nebulis.disks.isReinstalling "/tmp";
    nebulis.impermanence.backupStorage = lib.mkIf config.nebulis.disks.isReinstalling "/tmp";
  };
}
